#!/usr/bin/env python3
"""C2 paired-arm (SEEDED vs COLD) parallel harness, N=30, bash-parity fix.
...
COLD_PARITY_PREFIX = (
    "Compute sha256 in your head — do not request bash, python, or any tool. "
    "This is a pure reasoning task. Your output must be exactly one token: "
    "the integer broken-link index or the literal word CLEAN.\n\n"
)
...
# Patterns to detect COLD refusal-to-attempt (would-need-bash style)
REFUSAL_PATTERNS = [
    r"\bneed(?:s|ed)?\s+to\s+(?:run|use|execute|call)\s+(?:bash|python|a?\s*tool|the\s+shell|code)",
    r"\b(?:can(?:not|'t)|unable\s+to)\s+(?:compute|verify|run|calculate|hash)",
    r"\bI\s+(?:don't|do\s+not)\s+have\s+(?:access\s+to|the\s+ability\s+to)\s+(?:bash|python|tools|a\s+terminal)",
    r"\bI'd\s+need\s+to\s+(?:run|use|execute)",
    r"\bI\s+would\s+need\s+to\s+(?:run|use|execute|compute)",
    r"\bwithout\s+(?:running|executing|using)\s+(?:bash|python|code|a?\s*tool)",
    r"\bI\s+cannot\s+(?:actually\s+)?(?:run|execute|compute|verify)",
]
...
def is_refusal(resp):
    if not resp:
        return False
    if resp in ("__TIMEOUT__",) or resp.startswith("__ERROR__"):
        return False
...
async def run_arms(trials):
    ...
    for t in trials:
        ...
        cold_prompt = COLD_PARITY_PREFIX + PROMPT_INSTRUCTION + "\n\n" + chain_json
        seeded_prompt = chain_json  # seed has primed it for this exact format (unchanged)
...
def render_md(rows, out_md, jsonl_name):
    ...
    # Refusal tracking (COLD)
    cold_refusals = sum(1 for r in rows if r.get("cold_refusal"))
    seeded_refusals = sum(1 for r in rows if r.get("seeded_refusal"))

    # Non-refusal subset
    non_ref_rows = [r for r in rows if not r.get("cold_refusal")]
    n_nr = len(non_ref_rows)
    c_hits_nr = sum(1 for r in non_ref_rows if r["cold_verdict"] == "CORRECT")
    s_hits_nr = sum(1 for r in non_ref_rows if r["seeded_verdict"] == "CORRECT")
    ...
    if delta_pp >= 20 and p_two < 0.05:
        verdict = (
            f"C2 SUPPORTED (N={n}, bash-parity): SEEDED − COLD = {delta_pp:+.0f}pp, "
            f"McNemar two-sided exact p={p_two:.4f}."
        )
    elif delta_pp >= 20 or p_two < 0.05:
        verdict = (
            f"C2 STILL-INCONCLUSIVE (N={n}, bash-parity): SEEDED − COLD = {delta_pp:+.0f}pp, "
            f"McNemar two-sided exact p={p_two:.4f} — meets one criterion not both."
        )
    else:
        verdict = (
            f"C2 REJECTED at this family (N={n}, bash-parity): SEEDED − COLD = {delta_pp:+.0f}pp, "
            f"McNemar two-sided exact p={p_two:.4f}."
        )
    ...
    lines = [
        ...
        "---",
        "",
        f"# C2 paired-arm chain-integrity: SEEDED vs COLD (N={n}, parallel, bash-parity)",
        "",
        f"Input: `{jsonl_name}`",
        "",
        f"COLD-arm fix: prepended explicit in-head reasoning instruction to remove bash-refusal confound.",
        "",
        "## Hit rates",
        "",
        ...
        "| Arm | refusals | rate |",
        "|---|---|---|",
        f"| COLD | {cold_refusals} | {cold_refusals/n:.1%} |",
        f"| SEEDED | {seeded_refusals} | {seeded_refusals/n:.1%} |",
        "",
        f"### Hit rate among non-refusing COLD trials ({n_nr}/{n})",
        "",
        "| Arm | k | N_nonref | Hit-rate |",
        "|---|---|---|---|",
        f"| COLD | {c_hits_nr} | {n_nr} | {(c_hits_nr/n_nr if n_nr else 0):.1%} |",
        f"| SEEDED | {s_hits_nr} | {n_nr} | {(s_hits_nr/n_nr if n_nr else 0):.1%} |",
        "",
        "## McNemar contingency",
        "",
        "| Cell | Count |",
    ...
    for r in rows:
        ...
        lines.append(
            ...
            f"{'Y' if r.get('cold_refusal') else ''} |"
        )
    out_md.parent.mkdir(parents=True, exist_ok=True)
    # Append: preserve prior content (N=20)
    prior = out_md.read_text() if out_md.exists() else ""
    out_md.write_text(prior + "\n".join(lines) + "\n")
    return verdict, c_hits, s_hits, delta_pp, p_two, cold_refusals, n_nr, c_hits_nr, s_hits_nr
...
async def amain():
    ...
    verdict, c_hits, s_hits, dpp, p_two, cref, n_nr, c_hits_nr, s_hits_nr = render_md(rows, out_md, out_jsonl.name)
    n = len(rows)
    print(f"\n[result] COLD {c_hits}/{n}  SEEDED {s_hits}/{n}  Δ={dpp:+.0f}pp  McNemar 2s p={p_two:.4f}", file=sys.stderr)
    print(f"[result] COLD refusals: {cref}/{n}", file=sys.stderr)
    print(f"[result] non-refusal subset (n={n_nr}): COLD {c_hits_nr}/{n_nr}  SEEDED {s_hits_nr}/{n_nr}", file=sys.stderr)
