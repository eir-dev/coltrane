...
export async function runGig(
  ...
  function prepareChair(chair: Chair, phaseName: string): PreparedChair {
    ...
    if (chair.input_contract.length > 0) {
      ...
      const entryChair = phaseName === firstPhase?.name && chair.depends_on.length === 0;
    ...
    return { chair, phaseName, agent, primitive, domain_type, output_specs, inputs, skills, ...(reservedCost !== undefined ? { cost: reservedCost } : {}) };
  }
