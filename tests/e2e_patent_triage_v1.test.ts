...
// "Schema-valid" includes each output's CORE substance — a Signal names its source, an
// Interpretation states its claims, a Plan lists its steps, a Judgment names its criteria, an
// Artifact its validation_criteria, a Verdict its checks. outputs.write enforces all six on
// every seal (#227 ruling), so a stub that omits one aborts the chair, not just the assertion.
