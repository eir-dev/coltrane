...
describe("schema validation edge cases (adversarial — required-field-substance)", () => {
  ...
  it("required field set to null is REJECTED and error message names the failing field", async () => {
    ...
    // GREEN: the error projection now preserves instancePath (e.g. "/name") so
    // operators see the failing field — not just the type-class message.
    const namesField = /\bname\b/.test(res.error!);
    expect(namesField).toBe(true);
  });

  it("whole `data: null` at dispatchTool boundary reaches validate as null — boundary preserves caller intent", async () => {
    const deps = buildDeps();
    ...
    // GREEN: null is no longer silently coerced to {}. Ajv sees `null` and
    // rejects with "type: must be object" — the boundary no longer erases
    // the caller's signal before validation.
    expect(res.ok).toBe(false);
    expect(res.error).toBeDefined();
    expect(res.error!.toLowerCase()).toMatch(/must be object|invalid/);
  });
  ...
  // ───────────────────────────────────────────────────────────────────────────
  it("required string field set to 0 (number) is REJECTED with type-class message and names the field", async () => {
    const deps = buildDeps();
    const payload = basePayload();
    payload["name"] = 0;
    ...
    // GREEN: instancePath surfaces the failing field.
    expect(/\bname\b/.test(res.error!)).toBe(true);
  });
  ...
  // ───────────────────────────────────────────────────────────────────────────
  it("required boolean field set to 'true' (string) is REJECTED with type-class message and names the field", async () => {
    const deps = buildDeps();
    const payload = basePayload();
    payload["flag"] = "true";
    ...
    expect(/\bflag\b/.test(res.error!)).toBe(true);
  });
  ...
  // ───────────────────────────────────────────────────────────────────────────
  it("extra fields NOT in schema are REJECTED (additionalProperties: false enforces the closed-world contract)", async () => {
    const deps = buildDeps();
    const payload = basePayload();
    ...
    // GREEN: additionalProperties:false rejects unknown fields; the error names
    // the offending property so typos are caught at the boundary.
    expect(res.ok).toBe(false);
    expect(res.error).toBeDefined();
    expect(res.error!.toLowerCase()).toMatch(/additional|must not have/);
    expect(res.error!).toMatch(/totally_unknown_field/);
  });
  ...
  // ───────────────────────────────────────────────────────────────────────────
  it("nested required field set to null is REJECTED and error message names the path", async () => {
    const deps = buildDeps();
    const payload = basePayload();
    ...
    expect(res.error).toBeDefined();
    expect(res.error!.toLowerCase()).toMatch(/must be string/);
    // GREEN: instancePath surfaces the nested field path.
    expect(/\bnested\b/.test(res.error!) || /\binner\b/.test(res.error!)).toBe(true);
  });
  ...
  //    no-field-name gap.
  // ───────────────────────────────────────────────────────────────────────────
  it("required string field set to an array is REJECTED with type-class message and names the field", async () => {
    const deps = buildDeps();
    const payload = basePayload();
    payload["name"] = ["a", "b", "c"];
