...
describe("chain_query primitives (T7) — failure_rate + cycle_lineage", () => {
  ...
  it("the underlying data exists to compute failure_rate(voice_id) over a window", async () => {
    ...
    // Voice emits 5 verdicts: 3 pass, 2 fail. Expected failure_rate = 0.4.
    const verdicts = [
      { pass: true,  target: "art-1" },
      { pass: false, target: "art-2" },
      { pass: true,  target: "art-3" },
      { pass: false, target: "art-4" },
      { pass: true,  target: "art-5" },
    ];
    for (const v of verdicts) {
      await writeOutput(deps, {
        ...
        data: {
          ...
          // a soft-verdict's KILLED ≈ pass=false. Mirror it explicitly for the
          // failure_rate computation below.
          pass: v.pass,
          target_ref: v.target,
        },
      });
    ...
    const failed = outs.filter((o) => o.data["pass"] === false).length;
  ...
  it("output_trace does NOT scope to a single gig — cross-gig ancestors leak in", async () => {
    ...
    // RED-honest finding: if a later gig cites an earlier gig's output as
    // input, output_trace surfaces it without flagging the gig boundary. The
    // v3.3 chain_query.cycle_lineage primitive needs to either scope to the
    // given event's gig OR explicitly label each node with its cycle so the
    // caller can split. Currently the caller gets a flat ancestor set with
    // no cycle-boundary marker beyond gig_id on each node.
    //
    // This assertion documents the gap. Once cycle_lineage is wired with
    // gig-scoping, flip the assertion to expect(otherGigNodes.length).toBe(0).
    expect(otherGigNodes.length).toBeGreaterThan(0);
  });
});
